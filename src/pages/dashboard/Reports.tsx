...
const Reports: React.FC = () => {
  ...
  // ---------- Regenerate a specific report (currently just re‑generates the whole project) ----------
  const regenerateReport = async (reportId: string) => {
    if (!selectedProject) return;
    setLoading((prev) => ({ ...prev, generating: true }));
    try {
      await ReportService.generate(selectedProject);   // could be a dedicated endpoint
  ...
  // ---------- Helper to get the project name from its ID (for display) ----------
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      {/* Card for generating new reports */}
      <Card className="border-gray-200 shadow-sm rounded-2xl">
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {/* Sample report button (opens in new tab) */}
              <Button
                variant="outline"
      ...
      {/* Preview modal – shows the PDF in an iframe */}
